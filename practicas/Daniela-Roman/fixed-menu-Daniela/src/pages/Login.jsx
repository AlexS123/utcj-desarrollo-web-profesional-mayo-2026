...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        setMensajeError(datos.mensaje || "Correo o contraseña incorrectos.");
    ...
    <>
      ...
      <div className="loginContainer">
        <div className="loginCard">
          <h1>Entrar al itinerario</h1>
          <p>Ingresa tus datos para ver y editar el viaje</p>
      ...
      {mostrarModalAcceso && (
        <div className="modalAccesoOverlay">
          <div className="modalAcceso">
            <button className="cerrarModalAcceso" onClick={() => setMostrarModalAcceso(false)}>×</button>
            <div className="modalAccesoIcono">!</div>
            <h2>{tipoMensaje === "logout" ? "Sesión cerrada" : "Inicia sesión"}</h2>
            <p>{mensajeAcceso}</p>
            <button className="btnModalAcceso" onClick={() => setMostrarModalAcceso(false)}>Aceptar</button>
          </div>
        </div>
      )}
    </>
